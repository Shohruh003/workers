...
function Header() {
    ...
    return (
      <Box sx={styles.header}>
         <Box sx={styles.headerSearch}>
          ...
          autoComplete="off"
        >
          <TextField sx={{color: 'white'}} id="outlined-basic" label="Глобальный поиск" size="small" variant="outlined" />
        </Box>

          <Box sx={isAdminEndpoint() ? styles.timeFilter : styles.adminHeader}>
              <span style={{fontWait: 700}}>по полу:</span>
              <Checkbox {...label} color="default" sx={{color: "white"}} />
    ...
    <Box sx={styles.timeFilter} style={isAdminEndpoint() ? { display: 'none' } : null}>
            ...
            <LocalizationProvider dateAdapter={AdapterDayjs}>
              <DemoContainer components={['TimePicker', 'TimePicker']}>
                <TimePicker
                  sx={{ width: '40px', minWidth: '50px', maxWidth: '50px' }}
    ...
    <Box style={isAdminEndpoint() ? { display: 'none' } : null}>
        <NavLink sx={styles.navlink} style={isDayEndpoint() ? {textDecoration: 'none', fontWait: 700, marginRight: '10px', borderBottom: '2px solid red', color: 'red'} : {textDecoration: 'none', color: "#F5F5F5", fontWait: 700, marginRight: '10px'}} to='/'>Сегодня</NavLink>
        <NavLink style={isMonthEndpoint() ? {textDecoration: 'none', fontWait: 700, marginRight: '10px', borderBottom: '2px solid red', color: 'red'} : {textDecoration: 'none', color: "#F5F5F5", fontWait: 700}} to='/month'>За месяц</NavLink>
    </Box>
